...
from transformers import AutoModel, AutoModelForMaskedLM, BertTokenizer


class ERNIE_MLM(nn.Module):
    def __init__(self, args):
        super(ERNIE_MLM, self).__init__()

        self.ERNIE_MLM = AutoModelForMaskedLM.from_pretrained('nghuyong/ernie-2.0-en')
        for param in self.ERNIE_MLM.parameters():
    ...
    def forward(self, arg, mask_arg, token_mask_indices, Token_id, len_comp, len_cont, len_expa, len_temp):
        out_arg = self.ERNIE_MLM(arg, mask_arg)[0].cuda()  # [batch, arg_len, vocab]
